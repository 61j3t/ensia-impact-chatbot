...
- **External web pages (URLs shared in chat)**: `scripts/06_fetch_chat_links.py`. Reads every URL appearing in any message, drops obvious dead-ends (LinkedIn / Instagram / Facebook / YouTube / Google Forms / Discord / Telegram / etc.), and fetches each surviving URL **once**. **No site crawling** — we trust the chat's editorial signal that the specific page someone shared is what matters; the rest of the site wasn't curated by anyone (`MAX_DEPTH = 0`). Two backends per page: **httpx + trafilatura** first (fast static HTML), **Playwright headless Chromium** as fallback for JS-rendered sites and anti-scraper WAFs. Browser is launched once and reused. Output: `data/external_text/chat_links/<host>/<slug>.txt` with a per-page header (Source URL, Title, Backend, Fetched timestamp) and a `_manifest.json` for incremental re-runs. Latest run: 82 pages across 57 hosts, ~300 KB of text.
